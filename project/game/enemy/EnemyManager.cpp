...
void EnemyManager::SpawnBossMinions(int spawnCount, const Vector3 &summonCenter, Camera *camera, MapManager *mapManager) {
	...
	for (int i = 0; i < actualSpawnCount; ++i) {
		...
		for (int j = 0; j < 60; j++) {
			...
			float speed = (j % 2 == 0) ? 0.35f : 0.55f; // 内外2重リング
			float sc = (j % 2 == 0) ? 3.0f + (rand() % 6) * 0.3f : 4.5f + (rand() % 6) * 0.3f;
			Vector4 rc = (j % 2 == 0)
				? Vector4{ 0.9f, 0.5f, 1.0f, 0.9f }   // 明るい紫白（内）
				: Vector4{ 0.5f, 0.0f, 0.7f, 0.85f };  // 暗い紫（外）
		...
		for (int j = 0; j < 4; j++) {
			...
			float flashScale = 7.0f + (rand() % 6) * 0.5f; // 7.0~9.5 超大型フラッシュ
