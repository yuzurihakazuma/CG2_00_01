...
void BossClawEffect::Update(Player* player, EnemyManager* enemyManager, Boss* boss, Boss* extraBoss, const Vector3& bossPos, const LevelData& level){
			...
			if ( timer_ == 12 ) {
				...
				for ( int i = 0; i < 8; i++ ) {
					...
					GPUParticleManager::GetInstance()->Emit(
						...
						BossVisualColor::Secondary(casterBoss_, 0.9f));
				}
			}

			// -----------------------------------------------
			// 飛翔中トレイル: 進行方向の逆に流れるボス色の残像
			// -----------------------------------------------
			{
				// 進行方向の逆ベクトル (後方へ流す)
				Vector3 backDir = {
				...
				for ( int i = 0; i < 4; i++ ) {
					...
					GPUParticleManager::GetInstance()->Emit(
						...
						BossVisualColor::Primary(casterBoss_, 0.55f));
				}
			}
	...
	if ( timer_ >= 45 ) {
		...
		if ( canRepeatClaw ) {
			...
			casterPos_ = casterBoss_->GetPosition();
			...
			casterBoss_->SetActionLock(45);
			casterBoss_->StartClawRepeatWarning(22);
			return;
		}
