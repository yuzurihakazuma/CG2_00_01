...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // [KazumimiEngine patch] FLIPモデルのスワップチェーンは _SRGB フォーマットを直接指定できず
    // CreateSwapChainForHwnd が失敗する（ウィンドウを外へドラッグした瞬間の assert の原因）。
    // バッファは UNORM で作り、ガンマ補正は RTV 側を SRGB にして行う
    if (sd1.Format == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB) { sd1.Format = DXGI_FORMAT_R8G8B8A8_UNORM; }
    else if (sd1.Format == DXGI_FORMAT_B8G8R8A8_UNORM_SRGB) { sd1.Format = DXGI_FORMAT_B8G8R8A8_UNORM; }
    ...
    if (vd->SwapChain)
    {
        ...
        for (UINT i = 0; i < bd->numFramesInFlight; i++)
        {
            ...
            // [KazumimiEngine patch] スワップチェーンはUNORMだがRTVはSRGBで作る（ガンマ補正維持）
...
static void ImGui_ImplDX12_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
    ...
    if (vd->SwapChain)
    {
        ...
        {
            vd->SwapChain->GetBuffer(i, IID_PPV_ARGS(&back_buffer));
            // [KazumimiEngine patch] スワップチェーンはUNORMだがRTVはSRGBで作る（生成時と同じ）
            D3D12_RENDER_TARGET_VIEW_DESC rtv_desc = {};
            rtv_desc.Format = bd->RTVFormat;
            rtv_desc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;
            bd->pd3dDevice->CreateRenderTargetView(back_buffer, &rtv_desc, vd->FrameCtx[i].RenderTargetCpuDescriptors);
